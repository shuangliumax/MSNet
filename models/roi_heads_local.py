...
def afd(fm_s, fm_t, eps=1e-6):
    ...
    attention = nn.Sequential(
        nn.Conv2d(in_channels, mid_channels, 1, 1, 0, bias=True).cuda(),
        nn.ReLU(inplace=True),
        nn.Conv2d(mid_channels, in_channels, 1, 1, 0, bias=True).cuda()
    )
...
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets, s_feat_0, m_feat_0, l_feat_0):
    ...
    sigmoid_x = class_logits.sigmoid()
    ############################
    loss_map = 1 - (2 * sigmoid_x - 1).abs().pow(2)
    loss_map = loss_map.mean()
    ##############################################
    loss1 = afd(s_feat_0, l_feat_0)
    loss2 = afd(m_feat_0, l_feat_0)
    # classification_loss = F.cross_entropy(class_logits, labels) + loss_map + loss1 + loss2
    ...
    return classification_loss, box_loss, loss_map, loss1, loss2
...
class RoIHeads(nn.Module):
    ...
    def forward(
        ...
        features1 = dict(islice(features.items(), 3))
        box_features = self.box_roi_pool(features1, proposals, image_shapes)
        ...
        if self.training:
            ...
            loss_classifier, loss_box_reg, loss_map, loss_1, loss_2 = fastrcnn_loss(class_logits, box_regression, labels, regression_targets, s_feat, m_feat, l_feat)
            losses = {"loss_classifier": loss_classifier, "loss_box_reg": loss_box_reg, "loss_map": loss_map, "loss_1": loss_1, "loss_2": loss_2}
        else:
