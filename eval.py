...
if __name__ == '__main__':
    ...
    def evaluate(
        ...
        ##################################################################
        # Calculate recall, precision, and F1-score at IOU threshold = 0.5
        iou_threshold = 0.5

        # Compute true positives and false negatives
        true_positives = 0
        false_negatives = 0
        for true, pred in zip(target, preds):
            ...
            if iou_matrix.numel() == 0:
                continue  # 跳过处理没有计算到 IoU 的情况

            max_iou_values, _ = iou_matrix.max(dim=1)
            true_positives += (max_iou_values >= iou_threshold).sum().item()
            false_negatives += (max_iou_values < iou_threshold).sum().item()

        # Compute precision, recall, and F1-score
        # precision = true_positives / (true_positives + (len(preds) - true_positives))
        recall = true_positives / (true_positives + false_negatives)
        # f1_score = 2 * (precision * recall) / (precision + recall)

        # metric_summary['precision@0.5'] = precision
        metric_summary['recall@0.5'] = recall
        # metric_summary['f1_score@0.5'] = f1_score
