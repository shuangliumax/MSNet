...
def set_infer_dir():
    ...
    """
    if not os.path.exists('Results/gao/vis'):
        os.makedirs('Results/gao/vis')
    num_infer_dirs_present = len(os.listdir('Results/gao/vis/'))
    next_dir_num = num_infer_dirs_present + 1
    new_dir_name = f"Results/gao/vis/res_{next_dir_num}"
...
def set_training_dir(dir_name=None):
    ...
    if not os.path.exists('Results/'):
        os.makedirs('Results/')
    if dir_name:
        new_dir_name = f"Results/{dir_name}"
        os.makedirs(new_dir_name, exist_ok=True)
        return new_dir_name
    else:
        num_train_dirs_present = len(os.listdir('Results/'))
        next_dir_num = num_train_dirs_present + 1
        new_dir_name = f"Results//res_{next_dir_num}"
        os.makedirs(new_dir_name, exist_ok=True)
        return new_dir_name
